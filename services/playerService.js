...
async function refundToVirtualBalance(walletAddress, amount, reason) {
  try {
    ...
    logger.info(
      `💰 REFUNDED ${formatUSDC(
        amount
      )} to ${walletAddress}. Reason: ${reason}`,
    ...
    global.refundMetrics.byReason[reason] =
      (global.refundMetrics.byReason[reason] || 0) + 1;
    return true;
  } catch (err) {
...
async function updatePlayerStats(players, roomData) {
  ...
  for (const player of players) {
    ...
    logger.info(
      `Stats updated: ${player.username} | winner=${isWinner} | cycle=${cycle._id}`
    );
  }
}
...
async function findPlayerActiveRoom(walletAddress) {
  try {
    ...
    for (const roomId of roomIds) {
      ...
      if (player) {
        logger.info(
          `[ROOM_SEARCH] Found player ${walletAddress} in room ${roomId}`
        );
    ...
    logger.error(
      `[ROOM_SEARCH] Error finding active room for ${walletAddress}:`,
      error
    );
    return null;
  }
...
async function handlePlayerLeftWin(
  roomId,
  remainingPlayer,
  disconnectedPlayer,
  betAmount,
  ...
  try {
    ...
    if (
      ...
      if (!lockAcquired) {
        logger.info(
          `Tournament forfeit: duplicate processing blocked for match ${room.matchId}`
        );
      } else {
        ...
        if (winnerUser && loserUser) {
          try {
            ...
            if (claimed) {
              ...
              logger.warn(
                `[handlePlayerLeftWin] Match ${room.matchId} not claimed for room ${roomId} — skipping stats`
              );
            }
          } catch (e) {
