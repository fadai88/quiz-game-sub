...
class TournamentService {
  ...
  async createTournament(tournamentData) {
    try {
      const tournament = await Tournament.create({
        ...
        prizePool: {
          ...
          distribution:
            ...
            this._getDefaultPrizeDistribution(),
        },
        rules: {
      ...
      logger.info(
        `✅ Tournament created: ${tournament._id} - ${tournament.name}`
      );
      return tournament;
    } catch (error) {
  ...
  async registerForTournament(tournamentId, userId, walletAddress, username) {
    try {
      ...
      logger.info(
        `✅ User ${userId} registered for tournament ${tournamentId}`
      );
  ...
  async unregisterFromTournament(tournamentId, userId) {
    try {
      ...
      logger.info(
        `✅ User ${userId} unregistered from tournament ${tournamentId}`
      );
  ...
  async startTournament(tournamentId) {
    try {
      ...
      if (tournament.participants.length < tournament.minPlayers) {
        throw new Error(
          `Not enough participants. Minimum: ${tournament.minPlayers}, Current: ${tournament.participants.length}`
        );
      }
   ...
   */
  async processClaimedMatchResult(
    tournamentId,
    matchId,
    roomId,
    winnerUserId,
    ...
    try {
      ...
      if (match.status !== "in_progress" || match.roomId !== roomId) {
        logger.warn(
          `processClaimedMatchResult: match ${matchId} status=${match.status} roomId=${match.roomId} expected ${roomId} — rejected`
        );
        return { action: "match_recorded", claimed: false };
      }
      ...
      if (
        wId === lId ||
        !((wId === p1 && lId === p2) || (wId === p2 && lId === p1))
      ) {
        logger.warn(
          `processClaimedMatchResult: participant mismatch for match ${matchId} — expected [${p1},${p2}] got [${wId},${lId}]`
        );
        return { action: "match_recorded", claimed: false };
      }
      ...
      const winnerParticipant = tournament.participants.find(
        (p) => p.userId.toString() === winnerUserId.toString()
      );
      ...
      const loserParticipant = tournament.participants.find(
        (p) => p.userId.toString() === loserUserId.toString()
      );
      if (loserParticipant) {
        ...
        if (tournament.format === "single_elimination") {
          ...
          logger.info(
            `Player ${loserUserId} eliminated from tournament ${tournamentId}`
          );
        }
      }
      ...
      logger.info(
        `Tournament ${tournamentId}: ${activePlayers.length} active players remaining`
      );

      if (activePlayers.length <= 1) {
        logger.info(
          `Tournament ${tournamentId}: final match done, completing tournament`
        );
        await this.completeTournament(tournamentId);
        return { action: "tournament_complete", claimed: true };
      }

      const pendingMatchesInRound = currentRound.matches.filter(
        (m) => m.status !== "completed"
      ).length;
      if (pendingMatchesInRound === 0) {
        const nextRoundNumber = (currentRound.roundNumber ?? 0) + 1;
        logger.info(
          `Tournament ${tournamentId}: round ${currentRound.roundNumber} complete, generating round ${nextRoundNumber}`
        );
  ...
  async completeTournament(tournamentId) {
    try {
      ...
      for (let i = 0; i < tournament.prizePool.distribution.length; i++) {
        ...
        if (participant) {
          const prizeAmount =
            (tournament.prizePool.total * prize.percentage) / 100;
      ...
      logger.info(
        `✅ Tournament completed: ${tournamentId}, Winners: ${winners.length}`
      );
    ...
    for (const tournament of tournamentsToStart) {
      try {
        if (tournament.participants.length >= tournament.minPlayers) {
          ...
          logger.info(
            `✅ Auto-started tournament: ${tournament._id} - ${tournament.name}`
          );
        } else {
          logger.info(
        ...
        logger.error(
          `Failed to auto-start tournament ${tournament._id}:`,
          error
        );
      }
    }
  ...
  async _generateRoundMatchups(tournament, roundNumber) {
    try {
      ...
      logger.info(
        `Generated ${matches.length} matches for round ${roundNumber} of tournament ${tournament._id}`
      );
    } catch (error) {
