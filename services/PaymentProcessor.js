...
class PaymentProcessor {
  constructor(config) {
    ...
    for (const [prop, description] of Object.entries(requiredProps)) {
      if (!config[prop]) {
        throw new Error(
          `PaymentProcessor config missing required property: ${prop} (${description})`
        );
      }
    }
    ...
    if (
      ...
      config.rpcEndpoints.length === 0
    ) {
      throw new Error(
        "PaymentProcessor config.rpcEndpoints must be a non-empty array"
      );
    }
  ...
  async queuePayment(
    ...
    try {
      ...
      if (!this.config || !this.config.connection) {
        throw new Error(
          "PaymentProcessor config invalid - connection not available"
        );
      }

      // NEW: Pre-check treasury SOL balance (needs ~0.005 SOL for fees + ATA)
      const treasuryBalance = await this.config.connection.getBalance(
        this.config.TREASURY_WALLET
      );
      const minSol = 0.005 * 1e9; // 0.005 SOL in lamports
      if (treasuryBalance < minSol) {
        const error = new Error(
          `Treasury low on SOL: ${treasuryBalance / 1e9} SOL (needs at least ${
            minSol / 1e9
          })`
        );
        console.error({
  ...
  async processPaymentQueue() {
    ...
    try {
      ...
      if (!this.config || !this.config.connection || !this.config.io) {
        throw new Error(
          "PaymentProcessor config invalid - cannot process payments"
        );
      }
  ...
  async processPayment(payment) {
    ...
    if (!payment || typeof payment.markProcessing !== "function") {
      console.error({
        ...
        message:
          "This usually means the payment was fetched with .lean(). Check PaymentQueue.getPendingPayments().",
      });
      ...
      try {
        if (payment && payment._id) {
          ...
          if (
            freshPayment &&
            typeof freshPayment.markProcessing === "function"
          ) {
            console.log(
              "✅ Successfully refetched payment as Mongoose document"
            );
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `DB completion update failed: ${dbError.message}. Tx succeeded but status not updated.`
        );
      }
      ...
      if (this.config.io) {
        this.config.io
          .to(`wallet:${payment.recipientWallet}`)
  ...
  async sendPayment(payment) {
    const MAX_RETRIES = 3;
    let currentRetry = 0;
    let currentEndpointIndex = Math.floor(
      Math.random() * this.config.rpcEndpoints.length
    );
      ...
      try {
        ...
        if (payment.broadcastSignature) {
          ...
          if (result === "confirmed") {
            console.log({
              level: "info",
              event: "sendPayment",
              message: "Previous broadcast already confirmed",
              signature: payment.broadcastSignature,
            });
          ...
          console.log({
            ...
            event: "sendPayment",
            message:
              "Previous signature blockhash expired, building replacement transaction",
            signature: payment.broadcastSignature,
          });
        }
        ...
        if (!recipientTokenAccountInfo) {
          console.log({
            level: "info",
            event: "sendPayment",
            message: `Creating ATA for ${payment.recipientWallet}`,
          });
          transaction.add(
            createAssociatedTokenAccountInstruction(
              this.config.TREASURY_WALLET,
        ...
        console.log({
          level: "info",
          event: "sendPayment",
          message: "Getting latest blockhash...",
        });
        ...
        currentEndpointIndex =
          (currentEndpointIndex + 1) % this.config.rpcEndpoints.length;
        const newEndpoint = this.config.rpcEndpoints[currentEndpointIndex];
        console.log({
          level: "info",
          event: "sendPayment",
          message: `Switching RPC to ${newEndpoint}`,
        });
  ...
  async _confirmExistingSignature(connection, signature, lastValidBlockHeight) {
    ...
    while (Date.now() - startTime < MAX_TOTAL_MS) {
      ...
      if (lastValidBlockHeight !== null) {
        try {
          ...
          console.warn({
            level: "warn",
            event: "_confirmExistingSignature",
    ...
    const finalConfirmed = await this._checkSignatureStatus(
      connection,
      signature
    );
  ...
  async _checkSignatureStatus(connection, signature) {
    try {
      ...
      if (status.err)
        throw new Error(
          `Broadcast transaction failed on-chain: ${JSON.stringify(status.err)}`
        );
      return (
  ...
  async getWalletPayments(walletAddress) {
    return PaymentQueue.find({ recipientWallet: walletAddress }).sort({
      createdAt: -1,
    });
  }
  ...
  async testConfiguration() {
    ...
    const tests = [
      ...
      {
        name: "Treasury keypair exists",
        check: () => !!this.config?.TREASURY_KEYPAIR,
      },
      { name: "USDC mint exists", check: () => !!this.config?.USDC_MINT },
      {
        name: "Treasury wallet exists",
        check: () => !!this.config?.TREASURY_WALLET,
      },
      {
        name: "RPC endpoints array exists",
        check: () =>
          Array.isArray(this.config?.rpcEndpoints) &&
          this.config.rpcEndpoints.length > 0,
      },
      {
        name: "Can query payments",
        check: async () => {
    ...
    for (const test of tests) {
      try {
        const result =
          test.check instanceof Function ? await test.check() : test.check;
