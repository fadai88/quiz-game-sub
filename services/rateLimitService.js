...
async function rateLimitEvent(
  ...
  if (!limiter) {
    logger.error(
      `🚨 [RATE LIMIT] No limiter configured for event: ${eventName} — blocking by default`
    );
    if (socket) socket.disconnect(true);
    throw new Error(
      `Event "${eventName}" is not rate-limit configured. Connection closed.`
    );
  }
  ...
  try {
    ...
    if (ip) {
      try {
        ...
        logger.error(
          `🚨 [RATE LIMIT] IP ${ip} exceeded limit for ${eventName}`
        );
        if (eventName === "submitAnswer") {
          if (socket)
            socket.emit("answerError", "Too many submissions. Please wait.");
          throw new Error(
    ...
    if (error.msBeforeNext !== undefined) {
      ...
      logger.error(
        `🚨 [RATE LIMIT] ${walletAddress} exceeded limit for ${eventName}, retry in ${waitSeconds}s`
      );
      ...
      if (offenseCount > 5) {
        logger.error(
          `🚨 [SECURITY] ${walletAddress} is a repeat offender (${offenseCount} violations)`
        );
      ...
      throw new Error(
        `Rate limit exceeded for ${eventName}. Please wait ${waitSeconds} seconds.`
      );
    }
    throw error;
  }
  ...
  await safeRedisOp(
    async () => {
      ...
      if (parseInt(attempts) >= 5) {
        throw new Error(
          "Too many failed verification attempts. Try again in 1 hour."
        );
      }
...
async function isBlocked(identifier) {
  try {
    ...
    const byWallet = await context.redisClient.get(
      `blocklist:wallet:${identifier}`
    );
