...
async function verifyAndValidateTransaction(
    signature, expectedAmount, senderAddress, recipientAddress, nonce,
    ...
    try {
        const result = await TransactionLog.findOneAndUpdate(
            { signature },
            { $setOnInsert: { signature, walletAddress: senderAddress, betAmount: expectedAmount, verifiedAt: new Date(), status: 'verified' } },
    ...
    await safeRedisOp(async () => {
        ...
        if (exists) logger.info(`⚠️  Redis: Replay detected for ${key} (MongoDB already prevented)`);
    }, null, 'Redis signature check');
    ...
    try {
        ...
        if (storedNonce) {
            ...
            await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Nonce already used' });
        ...
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Nonce verification service unavailable' });
    ...
    try {
        ...
        if (error.message.includes('Invalid param: Invalid')) {
            ...
            await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Invalid signature' });
            throw new Error('Invalid transaction signature');
        }
        logger.error(`❌ Blockchain verification failed: ${error.message}`);
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: error.message });
        throw new Error('Failed to verify transaction on blockchain');
    }

    if (!transaction) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Transaction not found' });
    ...
    if (transaction.meta.err) {
        ...
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: JSON.stringify(transaction.meta.err) });
    ...
    if (senderIndex === -1) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Sender wallet not found in transaction' });
        throw new Error('Transaction sender verification failed');
    }
    const message = transaction.transaction.message;
    if (senderIndex >= message.header.numRequiredSignatures) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Sender did not sign transaction' });
    ...
    if (!postTokenBalances || !preTokenBalances) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Missing token balances' });
        throw new Error('Transaction missing required balance information');
    }

    const usdcMint = cfg.USDC_MINT.toBase58();
    const treasuryPost = postTokenBalances.find(b => b.owner === recipientAddress && b.mint === usdcMint);
    const treasuryPre  = preTokenBalances.find(b => b.owner === recipientAddress && b.mint === usdcMint);

    if (!treasuryPost || !treasuryPre) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Wrong token - expected USDC' });
        throw new Error('Transaction does not transfer USDC to treasury');
    }

    const actualTransferAmount = BigInt(treasuryPost.uiTokenAmount.amount || '0') - BigInt(treasuryPre.uiTokenAmount.amount || '0');
    if (!Number.isInteger(expectedAmount)) throw new Error('Bet amount must be in atomic units (integer)');

    if (actualTransferAmount !== BigInt(expectedAmount)) {
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: `Amount mismatch: expected ${formatUSDC(expectedAmount)}, got ${formatUSDC(actualTransferAmount)}` });
    ...
    try {
        ...
        if (!memoIx) {
            await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Missing memo instruction' });
            throw new Error('Transaction missing memo instruction for replay protection');
        }
        let memoText;
        try { memoText = Buffer.from(bs58.decode(memoIx.data)).toString('utf8'); } catch {
            try { memoText = Buffer.from(memoIx.data, 'base64').toString('utf8'); } catch { memoText = memoIx.data; }
        }
        if (!memoText.includes(nonce)) {
            await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Nonce mismatch in transaction memo' });
            throw new Error('Nonce mismatch - transaction does not match request');
        }
        logger.info(`✅ Memo nonce verified: ${nonce}`);
    } catch (error) {
        if (error.message.includes('Nonce') || error.message.includes('memo') || error.message.includes('MISSING')) throw error;
        await TransactionLog.findOneAndUpdate({ signature }, { status: 'failed', errorMessage: 'Invalid memo format' });
    ...
    try {
        ...
        logger.error('⚠️  Redis cache failed (non-blocking):', { error: err.message });
    }
