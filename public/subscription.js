...
async function subscribe(plan) {
  ...
  try {
    ...
    const amount =
      plan === "monthly" ? pricesData.prices.monthly : pricesData.prices.yearly;
    ...
    try {
      ...
      if (sendErr.message?.includes("already been processed")) {
        console.warn(
          "Transaction already processed — checking confirmation on chain."
        );
...
// Event listeners
document
  .getElementById("subscribeMonthlyBtn")
  .addEventListener("click", () => subscribe("monthly"));
document
  .getElementById("subscribeYearlyBtn")
