...
async function loadTournaments() {
  if (!hasPremium) {
    ...
    document.getElementById("upcomingTournamentsSection").style.display =
      "none";
  ...
  container.innerHTML = tournaments
    .map((tournament) => {
      const isRegistered = tournament.participants?.some(
        (p) => p.userId === userSubscription?.userId
      );
      ...
      return `
                    <div class="tournament-card">
                        <div class="tournament-header">
                            <div>
                                <div class="tournament-title">${esc(
                                  tournament.name
                                )}</div>
                                <p style="color: #94a3b8; font-size: 14px;">${esc(
                                  tournament.description
                                )}</p>
                            </div>
                            <div class="tournament-badge badge-${esc(
                              tournament.status
                            )}">
                        ...
                        ${
                          tournament.prizePool?.total > 0
                            ...
                            <div class="prize-pool">
                                ...
                                <div class="prize-amount">${Number(
                                  tournament.prizePool.total
                                )} ${esc(tournament.prizePool.currency)}</div>
                            </div>
                        `
                            : ""
                        }
        ...
        tournament.maxPlayers
                        ...
                        <div class="tournament-info">
                            ...
                            <div class="info-row">
                                ...
                                <span class="info-value">${
                                  tournament.rules?.questionsPerGame || 10
                                } per game</span>
                            </div>
                        </div>
                        ...
                        ${
                          !isHistory && now < startTime
                            ? `
                            <div class="countdown" id="countdown-${esc(
                              tournament._id
                            )}">
...
function getTournamentAction(
  ...
  if (isHistory) {
    ...
    const winner = tournament.winners?.find(
      (w) => w.userId === userSubscription?.userId
    );

    if (winner) {
      return `<button class="action-btn" disabled>🏆 Position ${Number(
        winner.position
      )} - Won ${Number(winner.prizeAmount)} USDC</button>`;
    }
    return `<button class="action-btn" disabled>Score: ${Number(
  ...
  if (tournament.status === "registration") {
    ...
    if (isRegistered) {
      return `<button class="action-btn unregister-btn" onclick="unregisterFromTournament('${esc(
        tournament._id
      )}')">Unregister</button>`;
    }
    if (isFull) {
    ...
    return `<button class="action-btn register-btn" onclick="registerForTournament('${esc(
      tournament._id
    )}')">Register Now</button>`;
  }
...
async function registerForTournament(tournamentId) {
  try {
    const response = await fetch(
      `${TOURNAMENTS_API}/${tournamentId}/register`,
      {
        method: "POST",
        credentials: "include",
      }
...
async function unregisterFromTournament(tournamentId) {
  ...
  try {
    const response = await fetch(
      `${TOURNAMENTS_API}/${tournamentId}/unregister`,
      {
        method: "POST",
        credentials: "include",
      }
