...
class AlertManager {
  ...
  async trackValidationFailure(ip, endpoint, errorDetails) {
    ...
    if (
      recentGlobalFailures.length >=
      ALERT_THRESHOLDS.VALIDATION_FAILURES_PER_HOUR.count
    ) {
  ...
  async sendGlobalAlert(failures) {
    ...
    // Log alert
    logger.error(
      "🚨 ALERT TRIGGERED: High Global Rate of Validation Failures",
      {
        category: "ALERT",
        ...alertData,
      }
  ...
  async sendAlert(alertType, identifier, counter, threshold) {
    if (!threshold) {
      logger.warn("sendAlert called with unknown alertType, skipping", {
        alertType,
        identifier,
      });
      return;
    ...
    if (alertType === "NOSQL_INJECTION_ATTEMPTS") {
      recommendation =
        "IMMEDIATE ACTION: IP auto-blocked and investigation required";
    ...
    if (shouldBlock) {
      try {
        ...
        logger.error(
          `🔒 AUTO-BLOCKED: ${identifier} for ${blockDuration}s (${alertType})`
        );

        // Close the Redis connection
        redisClient.disconnect();
      } catch (error) {
        logger.error("Failed to auto-block IP in Redis:", {
          error: error.message,
        });
      }
    }
  ...
  async sendExternalAlert(alertData) {
    ...
    console.log("\n" + "=".repeat(80));
    console.log(
      `🚨 SECURITY ALERT: ${alertData.alertTitle || alertData.alertType}`
    );
          ...
          attachments: [
            {
              ...
              fields: [
                ...
                {
                  title: "Blocked",
                  value: alertData.blocked
                    ? `Yes (${alertData.blockDuration}s)`
                    : "No",
                  short: true,
                },
                {
  ...
  cleanup() {
    ...
    for (const [key, counter] of this.alertCounters.entries()) {
      counter.events = counter.events.filter((e) => now - e.timestamp < maxAge);
  ...
  getStatus() {
    ...
    return {
      ...
      counters: Array.from(this.alertCounters.entries()).map(
        ([key, counter]) => ({
          key,
...
const trackRedisReconnect = (data) =>
  alertManager.track("REDIS_RECONNECT", "redis", data);
const trackMongoReconnect = (data) =>
  alertManager.track("MONGO_RECONNECT", "mongodb", data);
const trackFailedPayout = (data) =>
  alertManager.track("FAILED_PAYOUTS", "payments", data);
const trackStuckPayment = (data) =>
  alertManager.track("STUCK_PAYMENTS", "payments", data);
const trackLowTreasurySol = (data) =>
