...
try {
    ...
    console.warn('⚠️  sanitize-html not installed. Using basic fallback. Run: npm install sanitize-html');
    sanitizeHtml = (dirty) => {
...
function sanitizeError(error, context, userMessage = null) {
    ...
    if (isProduction) {
        return { error: userMessage || 'An error occurred. Please try again.', code: 'SERVER_ERROR', errorId };
    }
    return {
...
function sanitizeValidationError(validationError, context) {
    ...
    return { error: 'Validation failed', fields, code: 'VALIDATION_ERROR', errorId };
}

module.exports = { sanitizeForLog, sanitizeOutput, sanitizeText, sanitizeError, sanitizeValidationError };