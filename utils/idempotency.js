...
setInterval(() => {
  ...
  const retryRate =
    raceConditionMetrics.totalRetries / raceConditionMetrics.totalAttempts;
  ...
  if (retryRate > 0.1) {
    logger.warn(
      `⚠️ High race condition retry rate: ${(retryRate * 100).toFixed(2)}%`
    );
  }
  if (raceConditionMetrics.maxRetriesExceeded > 0) {
    logger.error(
      `❌ Max retries exceeded ${raceConditionMetrics.maxRetriesExceeded} times!`
    );
  }
}, 5 * 60 * 1000);
...
async function acquireIdempotencyLock(key, ttlSeconds = 30) {
  try {
    const result = await context.redisClient.set(
      key,
      "1",
      "NX",
      "EX",
