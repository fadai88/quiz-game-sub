...
router.post('/:id/register', authenticate, async (req, res) => {
    try {
        const ts = context.tournamentService;
        if (!ts) return res.status(503).json({ success: false, error: 'Tournament service not available' });
        const user = await User.findById(req.user.id);
        const tournament = await ts.registerForTournament(req.params.id, user._id, user.walletAddress, user.username);
...
router.post('/:id/unregister', authenticate, async (req, res) => {
    try {
        ...
        if (!ts) return res.status(503).json({ success: false, error: 'Tournament service not available' });
        const user = await User.findById(req.user.id);
        const tournament = await ts.unregisterFromTournament(req.params.id, user._id);
        res.json({ success: true, tournament });
    } catch (error) {
...
adminRouter.post('/:id/cancel', authenticate, requireAdmin, async (req, res) => {
    try {
        ...
        if (tournament.status === 'completed') return res.status(400).json({ success: false, error: 'Cannot cancel a completed tournament' });
