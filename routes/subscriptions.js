...
router.get("/prices", (req, res) => {
  try {
    ...
    logger.error("[SUBSCRIPTION] Error fetching prices:", {
      error: error.message,
    });
  ...
  try {
    ...
    if (user.subscriptionId) {
      ...
      if (sub && sub.status === "active" && endDate && endDate < new Date()) {
        ...
        tier = "free";
        logger.info(
          `[SUBSCRIPTION] Inline-expired subscription ${sub._id} for user ${user._id}`
        );
      }
    }
    ...
    logger.error("[SUBSCRIPTION] Error checking status:", {
      error: error.message,
    });
...
router.post("/cancel", authenticate, async (req, res) => {
  try {
    ...
    if (context.subscriptionService)
      await context.subscriptionService.cancelSubscription(user.subscriptionId);
...
router.post("/subscribe", authenticate, async (req, res) => {
  try {
    ...
    if (req.user.walletAddress !== walletAddress)
      return res
        .status(403)
        .json({ success: false, error: "Wallet address mismatch" });
    ...
    const subscription = await context.subscriptionService.createSubscription(
      user._id,
      walletAddress,
      transactionSignature,
      plan
    );
    logger.info("[SUBSCRIPTION] Created via REST:", {
      walletAddress,
      plan,
      transactionSignature,
    });
